export const normalizeServiceMonths = (value) => {
...
export const isElevatorDueThisMonth = (elevator, now = new Date()) => {
  if (!elevator) return false;

  const mode = normalizeScheduleMode(elevator.serviceScheduleMode);
  if (mode === 'months') {
    return normalizeServiceMonths(elevator.serviceMonths).includes(now.getMonth() + 1);
  }
  ...
  const computed = calculateNextServiceDateByMode({
    ...
    intervalServisa: Number(elevator.intervalServisa || 1),
    mode: 'interval',
    serviceMonths: [],
  });
