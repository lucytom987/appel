...
const serviceWorkOrderSchema = new mongoose.Schema({
...
serviceWorkOrderSchema.index({ serviceId: 1 }, { unique: true });
serviceWorkOrderSchema.index({ dayKey: 1, dailySequence: 1 }, { unique: true });
serviceWorkOrderSchema.index({ status: 1, updated_at: -1 });

module.exports = mongoose.model('ServiceWorkOrder', serviceWorkOrderSchema);
